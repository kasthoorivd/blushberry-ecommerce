...
const loadInventory = async (req, res) => {
  try {
    ...
    res.status(HttpStatus.OK).render('admin/inventory', {
      products: paginated,
      currentPage: page,
      totalPages,
      total,
    ...
    res.status(HttpStatus.INTERNAL_SERVER_ERROR).render('error', { message: 'Could not load inventory.' })
  }
}
...
const updateVariantStock = async (req, res) => {
  try {
    ...
    await product.save()
    return res.status(HttpStatus.OK).json({ success: true, message: 'Stock updated.', newStock })
  } catch (err) {
...
const getStockHistory = async (req, res) => {
  try {
    ...
    return res.status(HttpStatus.OK).json({ success: true, history, productName: product.name })
  } catch (err) {
...
module.exports = { loadInventory, updateVariantStock, getStockHistory }