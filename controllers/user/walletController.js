...
const getWallet = async (req, res) => {
    try {
        ...
        res.status(HttpStatus.INTERNAL_SERVER_ERROR).redirect('/profile')
    }
}
...
const debitWallet = async (userId, amount, description, orderId = null) => {
    ...
    wallet.balance -= amount
    wallet.transactions.push({ type: 'debit', amount, description, orderId, status: 'completed' })
    await wallet.save()
    return wallet
}
...
module.exports = {
    getWallet, creditWallet, debitWallet, getWalletBalance,
    handleCancellationRefund, handleReturnRefund, applyWalletPayment
}