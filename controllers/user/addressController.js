...
const loadAddresses = async (req, res) => {
  try {
    ...
    return res.status(HttpStatus.OK).render('user/addresses', {
      user,
      addresses,
      successMsg: null,
      errorMsg: null,
...
const loadAddAddress = (req, res) => {
  const user = req.session.user
  return res.status(HttpStatus.OK).render('user/addAddress', { user, errorMsg: null })
}
...
const editAddress = async (req, res) => {
  try {
    ...
    if (!name || !country || !state || !address || !pincode || !mobile || !email) {
      return res.status(HttpStatus.BAD_REQUEST).json({ success: false, message: 'Please fill in all required fields' })
    }

    if (!/^\d{6}$/.test(pincode)) {
      return res.status(HttpStatus.BAD_REQUEST).json({ success: false, message: 'Pin code must be a 6-digit number' })
    }

    if (!/^\d{10}$/.test(mobile)) {
      return res.status(HttpStatus.BAD_REQUEST).json({ success: false, message: 'Mobile number must be 10 digits' })
    }
...
const setDefaultAddress = async (req, res) => {
  try {
    ...
    return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ success: false, message: 'Failed to set default' })
  }
}
